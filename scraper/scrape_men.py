"""
Scraper for men's results from minfriidrettsstatistikk.info
...
# Correct class codes from website for MEN
MEN_CLASS_CODES = {
...
def scrape_landsstatistikk(class_code, season=2024, outdoor='Y'):
    ...
    class_info = MEN_CLASS_CODES.get(class_code, {})
    gender = class_info.get('gender', 'M')
...
def scrape_all_men(seasons=[2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024, 2025]):
    """Scrape all men's classes for given seasons."""
    all_results = []

    classes = list(MEN_CLASS_CODES.keys())
    total = len(seasons) * len(classes) * 2  # *2 for outdoor/indoor
    pbar = tqdm(total=total, desc="Scraping men's results")

    for season in seasons:
        for class_code in classes:
            for outdoor in ['Y', 'N']:
                venue = 'ute' if outdoor == 'Y' else 'inne'
                class_name = MEN_CLASS_CODES.get(class_code, {}).get('name', f'Class {class_code}')
...
def main():
    """Main function - scrape all men's data."""
    logger.info("Starting men's data scrape from minfriidrettsstatistikk.info")

    # Scrape all seasons
    results = scrape_all_men()

    logger.info(f"Scraped {len(results)} men's results")

    # Save raw results
    output_file = OUTPUT_DIR / 'men_results_raw.json'
    ...
    athletes_file = OUTPUT_DIR / 'men_athletes.json'
    with open(athletes_file, 'w', encoding='utf-8') as f:
        json.dump(athletes, f, ensure_ascii=False, indent=2, default=str)
    logger.info(f"Saved {len(athletes)} unique men athletes to {athletes_file}")
