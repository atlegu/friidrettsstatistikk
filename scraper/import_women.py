"""
Import women's data to Supabase.
Based on fast_import.py but uses women_results_raw.json
"""
...
def clean_performance(perf):
    ...
    # Time format: M.SS.cc -> total seconds
    if perf_str.count('.') == 2:
...
def batch_upsert_athletes(results):
    ...
    logger.info(f"Inserting {len(athletes)} women athletes...")
...
def main():
    logger.info("Loading women's data...")
    with open(DATA_DIR / 'women_results_raw.json', 'r', encoding='utf-8') as f:
        results = json.load(f)
    logger.info(f"Loaded {len(results)} women's results")
