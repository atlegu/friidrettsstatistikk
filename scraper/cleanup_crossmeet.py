"""
Cleanup cross-meet duplicates - same result registered under different meets.
Matches on (event_id, date, performance) regardless of meet_id.
"""
import time
...
def cleanup_crossmeet_duplicates(athlete_id):
    """Remove cross-meet duplicates for a single athlete."""
    all_results = []
    offset = 0
    while True:
        try:
    ...
    # Group by (event_id, date, performance) - ignoring meet_id
    seen = {}
    to_delete = []
    for r in all_results:
        key = (r['event_id'], r['date'], r['performance'])
        if key in seen:
...
for athlete_id in tqdm(all_athletes, desc="Cleaning"):
    deleted = cleanup_crossmeet_duplicates(athlete_id)
    if deleted > 0:
        total_deleted += deleted
        athletes_with_dupes += 1
    time.sleep(0.02)

print(f"\nTotal cross-meet duplicates deleted: {total_deleted}")
print(f"Athletes affected: {athletes_with_dupes}")
