...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return {
    title: `${club.name} - All-time lister`,
    description: `Historiske toppresultater for ${club.name} gjennom alle tider`,
  }
}

export default async function ClubAllTimePage({
  params,
  searchParams,
}: {
  ...
  const results = selectedEvent
    ? await getClubAllTimeResults(id, selectedEvent.id, gender, age, selectedEvent.result_type ?? "time")
    : []
  ...
  return (
    <div className="container py-6">
      ...
      <h1 className="mt-4 mb-4">{club.name} - All-time lister</h1>
